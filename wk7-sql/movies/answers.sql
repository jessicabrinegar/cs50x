...
-- in chronological order. Your query should output a table with two columns, one for the title
-- of each movie and one for the release year of each movie. You may assume that the title of
-- all Harry Potter movies will begin with the words “Harry Potter”, and that if a movie title
...
SELECT AVG(rating) FROM ratings, movies WHERE movie_id = id AND year = 2012;
...
WHERE movie_id = id AND year = 2010 
...
-- Your query should output a table with a single column for the title of each movie.
-- You may assume that there is only one person in the database with the name Johnny Depp & one named Helena Bonham Carter.
